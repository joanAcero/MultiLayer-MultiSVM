"""
exp9_scalability_timing.py — RECOVERY: SUSY only (MNIST already complete)
Also adds SUSY n_max=200k to avoid OOM during download.
"""
...
def run(log_path, csv_path, data_dir):
    ...
    try:
        banner("Exp9 Recovery — SUSY timing curves (MNIST already done)",
               "SUSY: n_max=200k (capped to avoid OOM during download)",
               f"n_train: {DATASETS['susy']['n_trains']}",
               f"P={P}  {N_SEEDS} seeds  Exact RBF SVM ≤ {RBF_N_LIMIT}")
        ...
        for tag, cfg in DATASETS.items():
            ...
            try:
                ...
                print(f"FAILED ({e}). Skipping."); continue
            ...
            print(f"{'━'*76}")
            print(f"  {'n_train':>9s}  {'model':<38s}  {'acc':>8s}  {'std':>6s}  {'time':>8s}")
            print(f"  {'─'*74}")

            for n_tr in cfg["n_trains"]:
                if n_tr >= n_pool: continue
